...
def send_file_contents_to_chat(file_path1, file_path2, file_path3, chat_history, mod_directory):
    ...
    if "3" in content3:
        ...
        error_message = "There is some syntax error" if "1" in content3 else "The trojan implementation is incorrect"
        prompt_content = f"Original Verilog Code -> {content2} Modified Verilog Code -> {content1} Issue: {error_message}"
